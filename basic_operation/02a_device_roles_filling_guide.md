...
本章不是附加材料，而是在角色还没有恢复成 `connected` 时必须先看的页面。它专门解决两个最常见的问题：
...
这份教程的作用就是：先帮你把角色绑定正确，再让这些直出命令真正可用。
...
2. 让工具根据固定身份恢复角色
3. 先确认角色、截图和当前端口都对上
4. 再复制这次检测生成的“可直接执行命令”
5. 执行完成后，再去看教学版参考命令理解参数来源
...
4. 重新运行检测确认角色恢复
5. 确认什么时候可以直接执行，什么时候要先停下来修角色
6. 再去改 LeRobot 命令里的当前端口
