...
本章的目标是先学会“看懂检测结果，再决定什么时候可以直接运行命令，什么时候必须先停下来确认”。
...
如果报告里的“可直接执行命令”已经生成完成，不代表你可以完全跳过理解。  
更推荐的课堂节奏是：先完成本节点的关键确认，再直接跑通，最后回看教学版命令。
...
6. 角色恢复成功后：
   - 要先确认：角色名、截图和当前端口是否一致
   - 要执行：复制“可直接执行命令”
   - 要讲解：再把 `<TOP_CAMERA_DEV>` 填进教学版参考命令
...
- 角色还是 `missing`
- 相机截图和 `top/wrist` 名字对不上
- 你看不懂当前 `tty/dev` 和固定身份 `by-id/by_path` 的区别
- `record`、`replay`、`rollout` 里出现了自动默认值，但你还没确认这些值是否符合本组实验

## 7. 双路径怎么选

- 想先沿着课程基线跑通：先确认关键提示，再复制 `可直接执行命令`
- 想让学生练习参数理解：执行后再对照 `教学版参考命令`
