...
本次课的目标是让学生在已经完成角色绑定和校准的基础上，优先使用检测工具给出的直接命令完成遥操作、录制和回放验证；如果需要教学解释，再回头看参考模板。
...
如果报告里的 `teleoperate` 已经生成“可直接执行命令”，优先直接复制那一段。  
下面保留的是教学版参考命令。
...
如果报告里的 `record` 已经生成“可直接执行命令”，可以直接复制执行；但请注意其中数据集名和任务描述可能是自动默认值，执行前要确认。
